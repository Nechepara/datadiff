...
- `Parse` performs no validation against either database — it is a pure string operation. A parsed name that matches nothing simply never reaches `tables.ignored` ([[#5.1 Rules that govern the report|§5.1]]).
...
- **An ignored table is described, not merely named.** The two members of `tables.ignored` are lists of a record carrying `Table`, `IgnoredBy` and `Reason`, never lists of strings ([[#5.1 Rules that govern the report|§5.1]]). `IgnoredBy` is an enum of two members, `System` and `User`, and it reaches the JSON as one of the two lowercase literals `"system"` and `"user"` — which is the one place the default options need more than the naming policy, so a `JsonStringEnumConverter` built with `JsonNamingPolicy.CamelCase` is registered on the cached instance described below. Serializing the enum as its numeric value would be a defect, not a stylistic difference: [[#5. Report structure|§5]] fixes the two spellings a consumer is entitled to switch on. `Reason` is carried in the result object itself rather than derived at serialization time, so a caller reading `TableStructureComparisonResult` without serializing it sees the same sentence the JSON would have shown. It takes one of **three** fixed values, not two: the enum and the sentence are not in bijection, because the two `system` cases are distinguished by the text alone ([[#5.1 Rules that govern the report|§5.1]]). The three sentences belong in one internal `static class` of `const string` fields, so that the golden files and the comparer quote one definition rather than two copies of a long literal.
- Every list is sorted ascending by its natural key (table name, then field name, then key name) with `StringComparer.Ordinal`, so that two runs over the same pair of databases produce byte-identical JSON. The natural key of a `tables.ignored` list is the entry's `table`, and it alone orders the list — `ignoredBy` and `reason` never participate, and they never need to, since a table has at most one entry per side ([[#5.1 Rules that govern the report|§5.1]]). Ordering deliberately does **not** use the comparer of [[#6.1 Database options, collation settlement and comparer detection|§6.1]]: a linguistic comparer orders through ICU, whose tables differ between host platforms and runtime versions, so the same databases would serialize differently on two machines. Ordinal ordering is safe here precisely because equality is still decided by the resolved comparer — two names it considers equal can never both appear in one list.
...
```json
{
  ...
  "tables": {
    ...
    "ignored": {
      "left": [
        {
          "table": "The name of a table fetched from the LEFT database that was left out of the comparison, `schema.table`",
          "ignoredBy": "system",
      ...
      "right": [
        {
          "table": "The name of a table fetched from the RIGHT database that was left out of the comparison, `schema.table`",
          "ignoredBy": "system",
          "reason": "The table was not found in the other database and was skipped by the system."
        },
        {
...
**Shape of an `ignored` entry.** `tables.ignored.left` and `tables.ignored.right` hold objects, not bare names. The block above shows several entries per side only to exhibit the possible values at once; a real list holds as many entries as that side has tables left out of the comparison, in any mix of the cases below, and is `[]` when it has none. Each object carries three string properties, and all three are written on every entry:

- `table` — the table's name as the side that owns the list holds it, rendered from `PreferredName` like every other identifier in the report (*Identifier form* below).
- `ignoredBy` — who took the table out of the comparison. Exactly two values exist, `system` and `user`. There is no third value, and the property is never absent.
- `reason` — a user-friendly sentence saying why the table was not compared. The text is a fixed literal selected by the case, never a message composed per table, so two runs phrase the same case identically and a consumer may group entries by it. There are **three** literals and only two `ignoredBy` values, so `ignoredBy` alone does not determine the text — the two `system` cases are told apart by `reason`, which is the finer of the two properties and the one a reader should quote:

| `ignoredBy` | `reason` | Written when | Paired with a `missings` entry |
| --- | --- | --- | --- |
| `system` | `The table was not found in the other database and was skipped by the system.` | The table exists on this side only: the other database holds nothing this run could have paired it with ([[#6.2 Building the table sets|§6.2]] step 7). | **Yes** — `missings.<opposite side>.tables` |
| `system` | `On the other side, the user excluded this table from comparison. The system did not find another candidate for the table. That's why it was skipped by the system during comparison.` | The other database *does* hold the table this one would have been compared against, but the caller excluded it there and no `Map` supplied this one with another partner, so nothing is left to compare it against (*An exclusion takes the partner out with it* below). | **No** — the table exists on both sides |
| `user` | `The table was skipped at the user's request.` | The caller excluded this side's table with `ConflictResolution.Ignore` ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]). | **No** |

The three strings are written exactly as printed above, punctuation and capitalization included. They are part of the report contract rather than prose, so an implementation that improves their wording breaks a consumer matching on them.

**Relationship between the table lists.** Every entry of `tables.ignored.left` and `tables.ignored.right` names a table of `tables.detected`, and the two lists together are the report's **only** record of a table that was fetched and then not compared — there is no second section, and no table leaves the comparison without an entry here. The lists are indexed by side, which is finer than the flat list they replace and matches exclusion being one-sided by construction ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]): an entry is recorded under the side whose database holds the table it describes. A table contributes at most one entry per side, and one name appearing in both lists is normal rather than a duplicate — the two entries describe two tables in two databases that happen to share a name. An `Ignore` entry that matched no table in the database it names contributes nothing and does not appear in the report. The comparison set is computed as `detected − ignored.left − ignored.right`.

**An exclusion takes the partner out with it, and the partner is the system's doing.** `Ignore` matches on the side it names and on that side only ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]), but a comparison needs two tables. When `Ignore(orders, From.Left)` matches the left `dbo.Orders`, the right database holds the table the run would have paired it with, and no `Map` has given that table another partner, the right copy has nothing left to compare against. The run takes it out too, and the two exclusions are recorded differently because they were made by different parties:

- `tables.ignored.left` gets `dbo.Orders` with `ignoredBy` = `user` — the caller named this table.
- `tables.ignored.right` gets `dbo.Orders` with `ignoredBy` = `system` and the second of the two `system` reasons, which says in as many words that the other side's copy was excluded by the user and no other candidate was found.

The right entry is `system` and not `user` because the caller never named the right table: the entry naming the left one says nothing about the right ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]), and it is the run, not the caller, that concludes the right copy has nothing left to compare against. **Neither entry is reported under `missings`**, and the second `system` reason is the one case where a `system` entry has no `missings` counterpart: the table exists in both databases, nothing about their structure differs on this point, and calling it missing on the side that plainly holds it would be false.

The rule is conditional on the partner being genuinely orphaned. A right-hand table that a `Map` bound into a forced pair keeps that pairing and is compared normally, so it appears in neither list — a forced pair is decided at [[#6.2 Building the table sets|§6.2]] step 3 and no exclusion on the other side disturbs it.

`Ignore(table)` is a different report, not the same one: it names both sides, so both entries are `user` and the second `system` reason never arises. This is the one place the shorthand and a single one-sided entry are distinguishable in the output, and it is a faithful distinction — the caller really did say two different things.

**`ignoredBy` records who acted on *this* table, and the caller acts first.** The caller's exclusions are applied at [[#6.2 Building the table sets|§6.2]] step 2, before the run ever asks which tables have a partner on the other side, so a table the caller excluded is recorded with `ignoredBy` = `user` even when it also happens to exist in one database only. `system` therefore means precisely *the caller did not name this side's table* — whether because no partner existed anywhere, or because the partner was taken away on the other side. `reason` is what separates those two, and a consumer that needs the distinction reads `reason` rather than inferring it from `ignoredBy`.

**Only tables can be ignored, and only tables have these lists.** The asymmetry the section used to carry is gone: `ConflictResolutionExtensions.Ignore` takes a `TableIdentity` and nothing else ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]), so a field or a key cannot be taken out of the comparison in the first place and there is nothing for a member-level ignored list to hold. Every exclusion the caller can express is therefore recorded in the report, and no object leaves the comparison without a trace. A caller who wants a particular field out of the report excludes the table that holds it, and `tables.ignored.<side>` says so with `ignoredBy` = `user`.

**A table found in only one database** lands in two sections at once: in `tables.ignored.<side where the table exists>` with `ignoredBy` = `system` and the plain *not found in the other database* reason, explaining why it was not compared, and in `missings.<opposite side>.tables`, recording the structural difference. The fields and keys of such a table are **not** additionally listed as missing. This is the only one of the three `tables.ignored` cases that describes a genuine structural difference, and so the only one that appears under `missings` at all.
...
2. Apply the one-sided entries, every one of which is of `Kind = Table` — there is no other kind of one-sided entry ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]). Match each such entry against its own side's table names with `StringComparer.Ordinal`, as [[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]] prescribes and never with the comparer of [[#6.1 Database options, collation settlement and comparer detection|§6.1]], and set the matched tables aside: each match becomes an entry of `tables.ignored` under **its own side**, carrying `ignoredBy` = `user` and the reason [[#5.1 Rules that govern the report|§5.1]] fixes for that value. This is a lookup per entry and needs no name-keyed dictionary of the tables themselves, which is why it can run before step 3. Running here is also what settles `ignoredBy`: a table set aside now already has its entry, so step 7 never revisits it and a table the caller excluded is never relabelled `system` on the grounds that it also lacked a partner. Keep the set of tables removed here, per side — step 7 reads it to decide which of the two `system` reasons a partnerless table on the opposite side gets, and the names it holds are the only way to tell a table that lost its partner to the caller from one that never had a partner at all.
...
7. Take the tables of the **left** database that step 2 did not exclude and step 3 did not bind into a forced pair, and that have no partner among the right database's tables under the resolved comparer. Every one of them gets an entry in `tables.ignored.left` with `ignoredBy` = `system`, and the two `system` reasons of [[#5.1 Rules that govern the report|§5.1]] are chosen by asking why the partner is absent: if the right database holds a table this one would have paired with and step 2 removed it, write the reason that names the user's exclusion on the other side; otherwise the right database genuinely has nothing to pair with, and the reason is the plain one. Answering that question is what the per-side set kept at step 2 is for — it is consulted here and nowhere else. `tables.ignored.right` is the mirror image. A table whose partner was taken by a forced pair gets the plain `system` reason like any other unmatched table: a forced pair is a pairing the caller asked for, not an exclusion, and the table really is left with nothing on the other side. The step only appends — the `user` entries step 2 wrote are already in these lists and are neither rewritten nor duplicated here.
8. Comparison set = `detected − ignored.left − ignored.right`, and every forced pair is in it.
...
10. Only one of the three `tables.ignored` cases feeds step 9, and it is selected by `reason` rather than by `ignoredBy` ([[#5.1 Rules that govern the report|§5.1]]). A table excluded by the caller is never reported as missing or inconsistent, even when it exists in one database only. Neither is a table carrying the `system` reason that names the user's exclusion on the other side — that table exists in both databases, so there is nothing missing to report. Only the plain `system` reason, *the table was not found in the other database*, describes a table that is genuinely one-sided, and that is the one step 9 pairs with a `missings.*.tables` entry.
...
| --- | --- | --- |
| 1 | `dbo.Orders` exists in one database only | `missings.<opposite side>.tables` **and** `tables.ignored.<side where it exists>`, the latter with `ignoredBy` = `system` and the plain *not found in the other database* reason |
...
- A table excluded with `Ignore` never appears anywhere in `difference`, and is recorded in `tables.ignored` with `ignoredBy` = `user` and the reason text [[#5.1 Rules that govern the report|§5.1]] fixes for that value. Asserted for `Ignore(table, From.Left)`, for `Ignore(table, From.Right)` and for `Ignore(table)`, each run twice. Against a fixture where **both** databases hold the table: the two one-sided calls produce a `user` entry on the side they name and a `system` entry carrying the *other side excluded it* reason on the side they do not, while `Ignore(table)` produces a `user` entry on each side and no `system` entry anywhere — the one shape in which the shorthand and a single one-sided entry yield different reports. Against a fixture where a **single** database holds it: the call naming that side produces one `user` entry and no counterpart on the empty side, while the call naming the empty side matches nothing and leaves the table an ordinary plain-`system` finding with its `missings` entry intact. Nothing lands under `missings` in any of the both-sides cases.
- **The three `reason` cases are asserted against one fixture.** Four tables: `dbo.Kept` in both databases and compared; `dbo.LeftOnly` in the left database alone; `dbo.Excluded` in both but named by `Ignore(excluded)`; and `dbo.Orders` in both but named by `Ignore(orders, From.Left)` only. The report carries `dbo.LeftOnly` under `tables.ignored.left` with `ignoredBy` = `system` and the plain reason, and under `missings.right.tables`; `dbo.Excluded` under both sides with `ignoredBy` = `user` and nothing under `missings`; and `dbo.Orders` under `tables.ignored.left` with `ignoredBy` = `user` and under `tables.ignored.right` with `ignoredBy` = `system` and the *other side excluded it* reason, again with nothing under `missings` — the case that proves a `system` entry does not imply a missing table. The test asserts every `reason` string byte-for-byte, that exactly three distinct strings occur, and that no entry carries an `ignoredBy` other than the two.
- **The caller's exclusion outranks the system's.** The fixture above run again with `Ignore(leftOnly, From.Left)` added: `dbo.LeftOnly` becomes a `user` entry rather than a plain-`system` one even though it still exists in one database only, and it leaves `missings.right.tables` — the caller's exclusion is applied first ([[#6.2 Building the table sets|§6.2]] step 2) and nothing relabels it afterwards.
- **A forced pair on the other side prevents the orphan rule from firing.** The left database holds `dbo.Orders` and `dbo.Legacy`, the right holds `dbo.Orders`, and the run carries both `Ignore(leftOrders, From.Left)` and `Map(leftLegacy, rightOrders)`. The right `dbo.Orders` is bound to `dbo.Legacy` at [[#6.2 Building the table sets|§6.2]] step 3, so it is compared rather than orphaned: it appears in neither `tables.ignored` list, and only the left `dbo.Orders` is ignored, with `ignoredBy` = `user`. Without the `Map`, the same fixture puts the right `dbo.Orders` in `tables.ignored.right` with the *other side excluded it* reason, which is what makes the pairing the operative difference.
...
- `Ignore` called with a `TableIdentity` spelling `dbo.orders` leaves `dbo.Orders` compared and absent from `tables.ignored`, and does so under a case-sensitive and a case-insensitive collation alike, since entries are matched ordinally and never with the run's comparer ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]). One entry per spelling excludes both. The case-insensitive half of this is the regression that guards the ordinal rule: a run whose collation calls the two names equal must still honour only the spelling the entry names.
...
- **Left set, `Right` `null`, `Kind = Table`.** Both databases hold `dbo.Table1`; an entry naming the left one excludes it. `dbo.Table1` appears in `tables.detected`, in `tables.ignored.left` with `ignoredBy` = `user`, and in `tables.ignored.right` with `ignoredBy` = `system` and the *other side excluded it* reason — the right copy having lost to the entry the only table it could have been compared against ([[#5.1 Rules that govern the report|§5.1]]) — nowhere under `missings`, and nowhere under `difference`, even though the two copies differ structurally, which the same fixture without the entry reports. The same fixture run with `Ignore(table1)` instead yields two `user` entries and no `system` one, and the two reports are compared against each other to pin that difference.
...
- **A wrong-case entry is ignored, on both sides, and this is the specified outcome rather than a tolerated defect.** The left database holds `dbo.Orders` and an entry's `Left` names `dbo.orders`: the entry is not taken into account for that table, `dbo.Orders` is compared as though no entry existed, it is absent from `tables.ignored`, and the call neither throws nor reports the entry anywhere. The mirror case — the right database holding `dbo.Orders` against an entry's `Right` naming `dbo.orders` — is asserted the same way. Both are run twice, under a case-sensitive collation and a case-insensitive one, to pin that the outcome does not depend on the collation.
...
2. **Golden-file tests** asserting the exact serialized JSON for a representative scenario, protecting the report contract against accidental change. At least one golden file is produced under a `PreferredCollation` and one without, so that the `collation` section is pinned on both paths; both assert that its four numeric members are JSON numbers rather than quoted strings, and a binary-collation case pins `"comparisonStyle": 0` as written rather than omitted. At least one golden file exercises all three `tables.ignored` cases at once, pinning that each entry is an object with all three properties, that `ignoredBy` is written as the bare literal `"system"` or `"user"` and never as a number, that each of the three `reason` sentences is byte-for-byte the one [[#5.1 Rules that govern the report|§5.1]] fixes — the long *other side excluded it* string included, whose punctuation and capitalization the golden file is the guard for — and that the two lists are ordered by `table` alone, with `system` and `user` entries interleaved by name rather than grouped. A second golden file pins the empty case, `"ignored": { "left": [], "right": [] }`, and that the `tables` section carries exactly two properties, `detected` and `ignored`, and no third table list of any name.
...
- `tables.ignored.left` and `tables.ignored.right` are the report's only record of a table that was fetched and not compared: each entry carries `table`, `ignoredBy` and `reason`, `ignoredBy` serializes as the literal `"system"` or `"user"` and never as a number, each `reason` is one of the three sentences [[#5.1 Rules that govern the report|§5.1]] fixes and is written exactly as fixed there, and a `missings.*.tables` counterpart exists for the plain `system` case alone — never for a `user` entry, and never for the `system` entry that records a partner excluded on the other side. The `tables` section carries exactly these two properties, `detected` and `ignored`, and the result model holds no third table list of any name.
