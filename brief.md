...
├── ConflictResolution               // the caller's exclusions and pairings (§3.1.1)
...
public sealed class TableStructureComparisonOptions
{
    ...
    /// an object excluded from the comparison (one side <c>null</c>), two names the run's
    /// collation cannot tell apart, or two objects the two databases spell differently
    /// because one of them was renamed. Empty by default — see §3.1.1 and §6.2.1.</summary>
...
public record ConflictResolution(INestedObject? Left, INestedObject? Right, ObjectKind Kind);
```

**How an entry is read.** `Left` names an object in the left database, `Right` an object in the right one, and `Kind` says what kind of object both denote. Either side may be `null`, and which of them is decides what the entry means:

| `Left` | `Right` | `Kind` | Interpretation |
| --- | --- | --- | --- |
| refers to `dbo.Table1` | `null` | `Table` | The table `dbo.Table1` of the **left** database is excluded from the structure comparison. |
| `null` | refers to `dbo.Table1` | `Table` | The table `dbo.Table1` of the **right** database is excluded from the structure comparison. |
| refers to `dbo.Table2` | refers to `dbo.table2` | `Table` | The left database's `dbo.Table2` is compared against the right database's `dbo.table2`. This is the answer to a collision the run's collation created — two spellings the library could not pair on its own ([[#6.2.1 Conflict analysis|§6.2.1]]). |
| refers to `dbo.Orders` | refers to `dbo.CustomerOrders` | `Table` | The left database's `dbo.Orders` is compared against the right database's `dbo.CustomerOrders`. This is the answer to a rename: one table under two names, which no comparison rule could ever pair. |
| refers to field `ModifiedOn` of `dbo.Tasks` | refers to field `Modified` of `dbo.Tasks` | `Field` | Field `ModifiedOn` of the left database's `dbo.Tasks` is compared against field `Modified` of the right database's `dbo.Tasks`. The same case one level down: a field renamed on one side. |
| `null` | `null` | any | Not a statement about anything. Rejected — see below. |

The same four shapes apply to `PrimaryKey` and `ForeignKey`, whose names are as renameable as a table's and as foldable by a collation.

**Exclusion is one-sided by construction, and that is the point.** A table lives in one database, and the reason to exclude it — it is EF's migration history, it is a staging table, its two names folded into one ([[#6.2 Building the table sets|§6.2]] step 5) — is a fact about that database, not about the pair. An entry that excludes `dbo.Audit` on the left says nothing about the right, so a caller who wants the table gone from both sides writes two entries. This is deliberate: under a case-sensitive collation the two databases may hold `dbo.Audit` and `dbo.audit`, and one entry that quietly matched both would be guessing.
...
**An object is named by at most one entry per side.** Two entries claiming the same left object, or the same right one, are contradictory and the library does not choose between them: the run fails with `InvalidConflictResolutionException` ([[#8. Error handling, cancellation, safety|§8]]). Two sides are "the same object" here by the same three things everything else in this section uses — `Kind`, `Name` and the owner chain, compared ordinally — so the check is a pure pass over the snapshot and costs no round trip. Because entries are matched against the databases by exactly that rule, the pass is also complete: two entries can claim one object only by naming it identically, so there is no second, later check to make when the entries are applied. An entry whose `Left` and `Right` are both `null`, and one whose non-null side carries a runtime type its `Kind` does not call for, fail the same way and are caught by the same pure pass.

**What the sides actually hold.** The runtime type follows `Kind`: a `TableIdentity` for `Table`, a `FieldIdentity` for `Field`, a `PrimaryKeyIdentity` for `PrimaryKey`, a `ForeignKeyIdentity` for `ForeignKey` — the identity objects the run already had in hand, handed over rather than copied into some reduced form. A conflict is about a name and its place in the database, which is exactly what an identity carries; the descriptor records built on top of them ([[#3.3.1 `IMetadataProvider`|§3.3.1]]) hold nothing a pairing decision could use. A caller answering an `AmbiguousNameException` can therefore build its `ConflictResolution` entries straight from the candidates it was given, with no reconstruction and no chance of mistyping a name, and can read the full context of each collision — a field's table, that table's schema — by walking `Owner`.
...
A caller keeps entries correct by not typing them. The candidates carried by an `AmbiguousNameException` **are** the objects they name ([[#6.2.1 Conflict analysis|§6.2.1]]), so entries built from them cannot be mis-spelled, and a `TableIdentity` obtained from `IMetadataProvider.GetTablesAsync` carries the server's own spelling. For entries that are typed by hand, `tables.detected` in the report ([[#5.1 Rules that govern the report|§5.1]]) is where a mis-spelled exclusion shows up — as a table that was compared after all.
...
```json
{
  ...
  "tables": {
    ...
    "unchecked": ["Tables a one-sided ConflictResolution entry excluded from the comparison"]
  },
  "difference": {
    "structure": {
      "missings": {
...
**Relationship between the table lists.** `tables.unchecked` is a subset of `tables.detected`, and holds the tables a one-sided `ConflictResolution` entry excluded ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]). An entry that matched no table in the database it names contributes nothing and does not appear in the report. The comparison set is computed as `detected − unchecked − ignored.left − ignored.right`.

**Only tables have an `unchecked` list.** An excluded field or key has no counterpart section: it is simply not compared, and produces no entry under `difference` on either side. The report therefore records *which tables* the caller took out of the comparison but not *which members*, which [[#11. Assumptions and open questions|§11]] flags as a point to confirm.
...
12. Apply the caller's entries of the member kinds — `Field`, `PrimaryKey`, `ForeignKey` — inside each pair of tables in the comparison set, exactly as steps 2 and 3 did for tables and under the same ordinal comparison: one-sided entries remove a member from the comparison, two-sided ones bind a forced pair that survives whatever the comparer would have said. An excluded member is compared against nothing and produces no entry anywhere in the report, since `tables.unchecked` has no counterpart below the table level ([[#11. Assumptions and open questions|§11]]). An entry whose owner names a table outside the comparison set is ignored, as its table was never going to be compared.
13. Run the **member stage** of the analysis ([[#6.2.1 Conflict analysis|§6.2.1]]) over the fields, primary keys and foreign keys that remain unpaired. As at step 4, the run either ends here with `AmbiguousNameException` or proceeds knowing every name pairs unambiguously. Only then does [[#6.3 Field comparison|§6.3]] begin.
...
**What clears a class.** A class disappears from this analysis when the caller's entries have emptied it of ambiguity — when every name on the side holding more than one has been either bound into a forced pair or excluded at [[#6.2 Building the table sets|§6.2]] steps 2, 3 and 12. A class reduced to one name per side is not a collision, so a caller may answer an `AmbiguousNameException` either by pairing the names or by excluding the ones it does not care about, and both readings are equally legitimate.
...
Within each table of the comparison set, fields are matched **by name**, using the comparer detected in [[#6.1 Database options, collation settlement and comparer detection|§6.1]]. Ordinal position is **not** compared. Fields the caller bound into a forced pair are already matched when this step begins and are not matched by name at all; fields the caller excluded are not here to match ([[#6.2 Building the table sets|§6.2]] step 12).
...
SQL Server allows at most one primary key per table, so only the outcome matters. The order of the fields inside the key is **not** taken into account — the participating fields are compared as a set under the detected comparer, and a field pair the caller forced counts as one field on both sides.
...
Foreign keys of a table are grouped by the pair `(table, referencedTable)`. A key the caller bound into a forced pair is paired before the grouping and skips the matching order below; one the caller excluded takes no part in it ([[#6.2 Building the table sets|§6.2]] step 12). Within each group the remaining left and right keys are matched in the following order:
...
- Table, field and key names are compared with the comparer detected in [[#6.1 Database options, collation settlement and comparer detection|§6.1]], never with a hard-coded one, and always through `IQuotedIdentifier.Name` — the bare, unquoted form, which is independent of the quoting policy in `TableStructureComparisonOptions`. Comparing `QuotedName` or `PreferredName` would make `[Orders]` and `Orders` two different tables. Objects the caller bound into a forced pair are the one exception: their pairing is fixed by the entry and the comparer is never asked about them ([[#6.2.1 Conflict analysis|§6.2.1]]).
...
- A table excluded by a one-sided `ConflictResolution` entry never appears anywhere in `difference`.
...
- An entry excluding `dbo.orders` leaves `dbo.Orders` compared and absent from `tables.unchecked`, and does so under a case-sensitive and a case-insensitive collation alike, since entries are matched ordinally and never with the run's comparer ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]). One entry per spelling excludes both. The case-insensitive half of this is the regression that guards the ordinal rule: a run whose collation calls the two names equal must still honour only the spelling the entry names.
...
- The same pair, run again with `ConflictResolution` containing the entry that pairs them, compares without error, and the report renders the left database's spelling of the paired table; run instead with a one-sided entry excluding one of the two colliding names, it also compares without error, with that name — and only that name — in `tables.unchecked`. Both assert the point of the ordinal rule: the entry singles out one member of a class whose members the run's own comparer cannot distinguish.
...
- That same fixture raises `ObjectLostException` at [[#6.2 Building the table sets|§6.2]] step 5, naming both table names, the left side, and the run's collation — and raises nothing once both names are excluded by a one-sided entry each.
...
- **A field exclusion.** A one-sided entry of `Kind = Field` removes that field from the comparison: it appears in no `missings` and no `inconsistencies` section, while the other fields of the same table are compared as usual.
- **Both sides `null`**, an entry whose runtime type contradicts its `Kind`, two entries claiming the same object on one side, and a two-sided entry matching on one side only each fail the call with `InvalidConflictResolutionException`; the first three do so before any metadata is read.
...
- **A conflict resolution the library cannot honour.** `InvalidConflictResolutionException` is thrown, carrying the offending entries, when the snapshot of `options.ConflictResolution` contradicts itself or contradicts the databases ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]): an entry with both sides `null`, an entry whose non-null side carries a runtime type its `Kind` does not call for, two entries claiming the same object on the same side, or a two-sided entry that matches an object on one side and none on the other. The first three are decided by validating the snapshot alone and are raised at [[#6.2 Building the table sets|§6.2]] step 0, which runs before any I/O; the last needs the metadata and is raised as soon as the entry is applied — step 3 for a table, step 12 for a member. An entry matching nothing on either side is not an error and raises nothing — including one whose spelling differs from the database's only in case, which applies to nothing and is passed over in silence ([[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]]). No partial report is produced in any case.
- **Ambiguous names.** When the conflict analysis of [[#6.2.1 Conflict analysis|§6.2.1]] ends with a non-empty candidate collection, `AmbiguousNameException` is thrown carrying it as an `IReadOnlyCollection<CandidateInfo>`, so the caller can see every collision at once and answer them in one edit to `TableStructureComparisonOptions.ConflictResolution` — by pairing the colliding names or by excluding them. No report is produced and no structure is compared. The exception can be raised at either of the analysis's two stages ([[#6.2.1 Conflict analysis|§6.2.1]]); reaching the second means the first found nothing left unresolved. It cannot be raised at all when `PreferredCollation` is unset, since a database does not hold two objects whose names its own collation calls equal.
- **A table lost to the comparison rule.** When the resolved comparer folds two table names of one and the same database into a single name, `ObjectLostException` is thrown ([[#6.2 Building the table sets|§6.2]] step 5). It carries the folded names, the side they came from, and the `CollationInfo` the comparer was resolved from, since that collation is the cause and the only thing the caller can change. It is not raised when every folded name was already excluded by a one-sided `ConflictResolution` entry or bound into a forced pair — either is the caller stating that the outcome is intended. Like `AmbiguousNameException` it is unreachable while `PreferredCollation` is unset, and it is raised before `tables.detected` exists, so no partial report is produced.
...
5. **Exclusion is specified for every kind, but only tables are reported as excluded.** The interpretation table of [[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]] gives exclusion examples for tables only. Since `Kind` is part of every entry and the analysis is kind-generic, the implementation accepts a one-sided entry of any kind and simply leaves that object out of the comparison. A table excluded this way is recorded in `tables.unchecked`; an excluded field or key has no such section and leaves no trace in the report at all ([[#5.1 Rules that govern the report|§5.1]]). Please confirm that silent member exclusion is wanted, or say whether member exclusions should be rejected outright, or recorded in a new report section of their own.
**Settled since an earlier draft.** How a `ConflictResolution` entry finds its object was an open question here and is one no longer: the comparison is `StringComparer.Ordinal` and nothing else, and an entry whose spelling does not match the database's — `dbo.orders` against a `dbo.Orders` the database actually holds — applies to nothing, on every collation, without an error and without a report entry of its own. That is confirmed expected behaviour and is specified in [[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]], not a trade still to be decided.
...
- The public API carries `ObjectKind`, `CandidateInfo`, `ConflictResolution`, `AmbiguousNameException` and `InvalidConflictResolutionException` as declared in [[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]], and `TableStructureComparisonOptions.ConflictResolution` is a get-only `HashSet<ConflictResolution>` that is empty by default, validated and snapshotted at the start of a call, and the **only** way a caller excludes an object or pairs two objects — there is no separate list of excluded table names.
- `ConflictResolution.Left` and `ConflictResolution.Right` are both `INestedObject?`. All four shapes of the interpretation table in [[#3.1.1 Conflicts: `CandidateInfo` and `ConflictResolution`|§3.1.1]] behave as tabulated — a one-sided entry excludes, a two-sided entry pairs and consumes both objects, whether the two names differ by case or are two different names after a rename, and whether they name tables or members — and each has an automated test of its own. An entry whose spelling does not match the database's, case included, applies to nothing: it raises no exception, appears nowhere in the report, and leaves its object compared as though the entry were absent.
...
// One side null excludes; both sides set pair.
...
var options = new TableStructureComparisonOptions
{
    ...
    ConflictResolution =
    {
        // Exclude dbo.__EFMigrationsHistory from the left database, then from the right.
        new ConflictResolution(migrations, null, ObjectKind.Table),
        new ConflictResolution(null, migrations, ObjectKind.Table),

        // The table was renamed on the right: compare the two as one table.
        new ConflictResolution(leftOrders, rightOrders, ObjectKind.Table),

        // A field of dbo.Tasks was renamed on the right: compare the two as one field.
        new ConflictResolution(
            new FieldIdentity(leftTasks, "ModifiedOn"),
            new FieldIdentity(rightTasks, "Modified"),
            ObjectKind.Field),

        // Entries answering an AmbiguousNameException go here too — its candidates say
        // which names need pairing, and carry the identities to pair (§3.1.1, §6.2.1).
    },
